...
@WebServlet("/food-group-names")
public class FoodProfilesDataServlet extends HttpServlet {

  @Override
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    Query query = new Query("FoodProfiles");
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    for(String pName: profileNameList){
        Entity foodEntity = new Entity("FoodProfiles");
        foodEntity.setProperty("Username", pName);
        datastore.put(foodEntity);
    }

    // Redirect back to the chat page.
    response.sendRedirect("/food-chat.html");
  }
}
